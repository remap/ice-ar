...
namespace Kalman
{
    public sealed class KalmanFilter
    {
        ...
        public void Correct (Matrix z)
		{
			...
			Debug.Log("kalman gain original = " + k.ToString());
			var g = 0.8 * Matrix.IdentityMatrix (k.rows, k.cols);
			Debug.Log("Kalman gain = " + g);
			State = X0 + (g * (z - (H * X0)));
			//Covariance = (Matrix.Identity (P0.RowCount) - k * H) * P0;
			Covariance = (Matrix.IdentityMatrix (P0.rows) - g * H) * P0;
		}
    }
}