...
// Wrapper<IceDatum> instead of Wrapper<op::Datum>
struct IceDatum : public op::Datum
{
    unsigned int frameNumber;

    IceDatum(unsigned int frameNumber = 0) :
...
// in this case we assume a std::shared_ptr of a std::vector of IceDatum

// This worker will constantly read from nanomsg socket
class WNanoInput : public op::WorkerProducer<std::shared_ptr<std::vector<IceDatum>>>
{
public:
    WNanoInput(const std::string& nanoPipe,
        const int frameWidth, const int frameHeight) :
        // this is replaced with reading from file for sample video test
        // mNanoSocket(ipc_setupSubSinkSocket(nanoPipe.c_str())),
        mNanoSocket(open(nanoPipe.c_str(), O_RDONLY)),
        mFrameWidth(frameWidth), mFrameHeight(frameHeight),
        mBufferSize(4*frameWidth*frameHeight),
        mFrameBuffer((unsigned char*)malloc(mBufferSize)),
        mFrameNumber(0)
    {
        if (mNanoSocket < 0)
            op::error("Failed to setup nanomsg socket: " + nanoPipe, __LINE__, __FUNCTION__, __FILE__);
    }

    void initializationOnThread() {}

    std::shared_ptr<std::vector<IceDatum>> workProducer()
    {
        try
        {
            unsigned int frameNumber;
            // reading from file instead of nano pipe for sample video test
            // int ret = ipc_readFrame(mNanoSocket, &frameNumber, mFrameBuffer, mBufferSize);
            int ret = read(mNanoSocket, mFrameBuffer, mBufferSize);

            if (ret < 0)
                op::error("Error reading from socket: "+std::string(ipc_lastError()), __LINE__, __FUNCTION__, __FILE__);
            else
            {
                // this is added for sample video test
                mFrameNumber++;
                frameNumber = mFrameNumber;

                std::stringstream ss;
                ss << frameNumber;
                op::log("Read frame "+ss.str(), op::Priority::High);

                // process frame
                auto datumsPtr = std::make_shared<std::vector<IceDatum>>();
                datumsPtr->emplace_back();
                auto& datum = datumsPtr->at(0);
                datum.frameNumber = frameNumber;

                cv::Mat argbImage = cv::Mat(cvSize(mFrameWidth,mFrameHeight), CV_8UC4, mFrameBuffer, cv::Mat::AUTO_STEP);
                cv::Mat bgrImage = cv::Mat(cvSize(mFrameWidth,mFrameHeight), CV_8UC3);
                static int mixChannelsIndices[] = {1,2, 2,1, 3,0};

                cv::mixChannels(&argbImage, 1, &bgrImage, 1, mixChannelsIndices, 3);
                cv::flip(bgrImage, bgrImage, -1);

                datum.cvInputData = bgrImage;

                if (datum.cvInputData.empty())
                {
                    op::error("Something is wrong - empty frame obtained", __LINE__, __FUNCTION__, __FILE__);
                    datumsPtr = nullptr;
                }
...
// images are dumped in BGR format
// to read/playback images:
//  $ ffplay -f rawvideo -vcodec rawvideo -s 320x180 -pix_fmt bgr24 -i /tmp/openpose-out
class WPipeOutput : public op::WorkerConsumer<std::shared_ptr<std::vector<IceDatum>>>
{
public:
    WPipeOutput(const std::string &previewFilePipe):
    mOutputFilePipe(previewFilePipe),
    mPipe(-1){}
    ...
    void workConsumer(const std::shared_ptr<std::vector<IceDatum>>& datumsPtr)
    {
        try
        {
            ...
            if (datumsPtr != nullptr && !datumsPtr->empty())
            {
                if (mPipe < 0)
                {
                    create_pipe(mOutputFilePipe.c_str());
                    mPipe = open(mOutputFilePipe.c_str(), O_WRONLY|O_NONBLOCK|O_EXCL);
                }
                ...
                else
                {
                    ...
                    writeExactly(image.data, image.elemSize()*image.total(), mPipe);
        ...
        {
            op::log("Unexpected error happened while writing to output file pipe");
            op::error(e.what(), __LINE__, __FUNCTION__, __FILE__);
        }
    }
private:
    std::string mOutputFilePipe;
    int mPipe;
};
...
public:
    ...
    {
        if (mNanoSocket < 0)
            op::error("Failed to setup nanomsg socket: " + nanoPipeFilename, __LINE__, __FUNCTION__, __FILE__);
    }
    ...
    void work(std::shared_ptr<std::vector<IceDatum>>& datumsPtr)
    {
        try
        {
            ...
            if (datumsPtr != nullptr && !datumsPtr->empty())
            {
                if (mNanoSocket > 0)
                {
                    std::vector<std::string> jsonsArray = buildJson(datumsPtr);
...
private:
    ...
    std::vector<std::string> buildJson(const std::shared_ptr<std::vector<IceDatum>>& datumsPtr){
        std::vector<std::string> jsonArray;

        for (auto i = 0u ; i < datumsPtr->size() ; i++)
        {
            const auto& tDatum = (*datumsPtr)[i];
    ...
    std::string saveKeypointsJson(unsigned int frameNo, const std::vector<std::pair<op::Array<float>, std::string>>& keypointVector)
    {
        op_ice::JsonOsstream jsonWriter(true);
        ...
        try
        {
            ...
            for (auto person = 0 ; person < numberPeople ; person++)
            {
                ...
                for (auto vectorIndex = 0 ; vectorIndex < keypointVector.size() ; vectorIndex++)
                {
                    ...
                    jsonWriter.key(keypointName);
                    jsonWriter.arrayOpen();
                    // Body parts
                    for (auto bodyPart = 0 ; bodyPart < numberBodyParts ; bodyPart++)
                    {
                        const auto finalIndex = 3*(person*numberBodyParts + bodyPart);
                        jsonWriter.plainText(keypoints[finalIndex]);
                        jsonWriter.comma();
                        jsonWriter.plainText(keypoints[finalIndex+1]);
                        jsonWriter.comma();
                        jsonWriter.plainText(keypoints[finalIndex+2]);
                        if (bodyPart < numberBodyParts-1)
                            jsonWriter.comma();
                    }
                    jsonWriter.arrayClose();
                    if (vectorIndex < keypointVector.size()-1)
                        jsonWriter.comma();
                }
                jsonWriter.objectClose();
...
int openPoseTutorialWrapper2()
{
    ...
    // Configure wrapper
    // @peetonn TODO: try with op::WrapperStructOutput{} to disable default output
    opWrapper.configure(wrapperStructPose, wrapperStructFace, wrapperStructHand, op::WrapperStructExtra{}, op::WrapperStructInput{},
                        wrapperStructOutput);
